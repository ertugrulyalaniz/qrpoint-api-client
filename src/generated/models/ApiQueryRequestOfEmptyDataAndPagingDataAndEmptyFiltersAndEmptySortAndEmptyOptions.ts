/* generated using openapi-typescript-codegen -- do not edit */
/* istanbul ignore file */
/* tslint:disable */
/* eslint-disable */
import type { EmptyData } from './EmptyData';
import type { EmptyFilters } from './EmptyFilters';
import type { EmptyOptions } from './EmptyOptions';
import type { EmptySort } from './EmptySort';
import type { PagingData } from './PagingData';
export type ApiQueryRequestOfEmptyDataAndPagingDataAndEmptyFiltersAndEmptySortAndEmptyOptions = {
    data: (null | EmptyData);
    paging: (null | PagingData);
    filters: (null | EmptyFilters);
    sort: (null | EmptySort);
    options: (null | EmptyOptions);
};

